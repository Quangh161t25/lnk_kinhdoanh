  ...
  const html = `
        <div style="padding: 0 0 16px 0;">
          <div class="field" style="margin-bottom:12px;">
            <label for="oldPassword" style="display:block;margin-bottom:6px;font-weight:500;font-size:14px;">Mật khẩu hiện tại</label>
            <input type="password" id="oldPassword" placeholder="Nhập mật khẩu hiện tại" autocomplete="new-password" style="width:100%;padding:10px 12px;border:1px solid var(--line);border-radius:8px;outline:none;font-family:inherit;font-size:14px;" />
          </div>
          <div class="field" style="margin-bottom:12px;">
            <label for="newPassword" style="display:block;margin-bottom:6px;font-weight:500;font-size:14px;">Mật khẩu mới</label>
            <input type="password" id="newPassword" placeholder="Nhập mật khẩu mới" autocomplete="new-password" style="width:100%;padding:10px 12px;border:1px solid var(--line);border-radius:8px;outline:none;font-family:inherit;font-size:14px;" />
          </div>
          <div class="field" style="margin-bottom:12px;">
            <label for="confirmPassword" style="display:block;margin-bottom:6px;font-weight:500;font-size:14px;">Nhập lại mật khẩu mới</label>
            <input type="password" id="confirmPassword" placeholder="Nhập lại mật khẩu mới" autocomplete="new-password" style="width:100%;padding:10px 12px;border:1px solid var(--line);border-radius:8px;outline:none;font-family:inherit;font-size:14px;" />
          </div>
          <div style="text-align:right; margin-top: 24px;">
            <button type="button" id="btnCancelCp" class="btn" style="background:#f1f5f9;color:#334155;margin-right:8px;border:none;padding:10px 18px;border-radius:8px;cursor:pointer;font-weight:600;font-family:inherit;font-size:14px;">Hủy</button>
            <button type="button" id="btnSubmitCp" class="btn btn-primary" style="background:#2d5bff;color:#fff;border:none;padding:10px 18px;border-radius:8px;cursor:pointer;font-weight:600;font-family:inherit;font-size:14px;">Xác nhận đổi</button>
          </div>
        </div>
      `;
  openModal("Đổi mật khẩu", "Cập nhật mật khẩu đăng nhập của bạn", html);

  document.getElementById("btnCancelCp").onclick = closeModal;
  document.getElementById("btnSubmitCp").onclick = async () => {
    const oldPw = document.getElementById("oldPassword").value;
    const newPw = document.getElementById("newPassword").value;
    const confirmPw = document.getElementById("confirmPassword").value;

    if (!oldPw || !newPw || !confirmPw) return showToast("Vui lòng điền đủ thông tin!", true);
    if (oldPw !== currentUser.mk) return showToast("Mật khẩu hiện tại không đúng!", true);
    if (newPw !== confirmPw) return showToast("Mật khẩu mới không khớp!", true);
    if (newPw.trim().length < 4) return showToast("Mật khẩu mới phải có ít nhất 4 ký tự!", true);

    const btn = document.getElementById("btnSubmitCp");
    const originBtnText = btn.textContent;
    btn.textContent = "Đang xử lý...";
    btn.disabled = true;
    btn.style.opacity = 0.7;

    try {
      const rawValues = await fetchSheetValues(CONFIG.authSheetName);
      const rowIndex = rawValues.findIndex(row => String(row[0] || "").trim() === currentUser.id);
      if (rowIndex === -1) throw new Error("Không tìm thấy tài khoản trong dữ liệu mạng");

      while (rawValues[rowIndex].length <= 6) rawValues[rowIndex].push("");
      rawValues[rowIndex][6] = newPw.trim();

      await updateSheet(CONFIG.authSheetName, rawValues);

      currentUser.mk = newPw.trim();
      const userObj = usersData.find(u => u.id === currentUser.id);
      if (userObj) userObj.mk = newPw.trim();
      ...
      showToast(err.message || "Lỗi khi đổi mật khẩu", true);
    } finally {
      btn.textContent = originBtnText;
      btn.disabled = false;
      btn.style.opacity = 1;
    }
  };
